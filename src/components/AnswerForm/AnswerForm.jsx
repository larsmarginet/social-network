  ...
  const handleFormSubmit = (e, comment) => {
    ...
    return useObserver(() => (
        <form className={comment.answerForm ? styles.answer__form : styles.answer__formClosed} onSubmit={e => handleFormSubmit(e, comment)}>
            ...
            <label className={styles.answer__form__label}>Answer<span className={styles.answer__form__user}>@{comment.name.replace(/ /g, '')}</span><textarea value={text} className={`${styles.answer__form__input} ${styles.answer__form__inputComment} ${styles.comment}`} rows="4" onChange={e => setText(e.currentTarget.value)}></textarea></label>
